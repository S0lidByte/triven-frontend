...
<script lang="ts">
	...
	const isRootApiKey = $derived(
		Array.isArray(config.path) &&
			...
			config.path[0] === "api_key"
	);

	const inputType = $derived(revealed ? "text" : "password");
	const attributes = $derived(
		inputAttributes(ctx, config, "shadcn4ApiKey", handlers, {})
	);
...
<div class="flex flex-col gap-1.5">
	<div class="border-input ring-offset-background flex min-w-0 items-center gap-1 rounded-md border shadow-xs focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
		<input
			bind:value
			{...inputAttrs}
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				This will generate a new API key and invalidate the current one. You will need to update any applications using the old key. Continue?
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
